...
## 400]. We add two other modes of functioning above. This aims at
## highlighting the application-dependent trade-off of fairness.
## Here, fairness factor is 0.2 .
...
NB_QUERY = 1500
FAIRNESS = 0.2
EXPORT_TRACES_FILE = Path(f'../../results/result_fairness_3b_s{SEED}.json')
...
def getInputs ():
    low = I.closed(51, 100)
    medium = I.closed(201, 250)
    high = I.closed(351, 400)
    veryHigh = I.closed(501, 550)
    crazyHigh = I.closed(651, 700)

    randoms = []
    randoms.append(boxes.getInput(low | medium))
    randoms.append(boxes.getInput(low | medium))
    randoms.append(boxes.getInput(low | medium | high))
    randoms.append(boxes.getInput(medium | high | veryHigh))
    randoms.append(boxes.getInput(high | veryHigh | crazyHigh))
    return randoms
